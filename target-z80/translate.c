...
//                    break;
                ...
                case 7:
                    ...
                    break;
#if 1	/* WmT: HACK */
                default:	/* switch(y) incomplete */
;DPRINTF("[%s:%d] FALLTHROUGH - unprefixed [MODE_%s] op 0x%02x (x %d, y %d [p=%d/q=%d], z %d) - unhandled y case\n", __FILE__, __LINE__, (m == MODE_NORMAL)?"NORMAL":"xD", b, x, y,p,q, z);
                    goto illegal_op;
#endif
                }
                break;
...
#if 1	/* WmT - HACK */
;DPRINTF("[%s:%d] HACK - PREFIX_CB case, byte 0x%02x (x %d, y %d [p=%d/q=%d], z %d) -> unhandled\n", __FILE__, __LINE__, b, x, y,p,q, z);
;goto illegal_op;
#else
;DPRINTF("[%s:%d] PARTIAL - PREFIX_CB case, byte 0x%02x (x %d, y %d [p=%d/q=%d], z %d) retrieved\n", __FILE__, __LINE__, b, x, y,p,q, z);
#endif
//        if (m != MODE_NORMAL) {
...
#if 1	/* WmT - HACK */
;DPRINTF("[%s:%d] HACK - PREFIX_ED case, byte 0x%02x (x %d, y %d [p=%d/q=%d], z %d) -> unhandled\n", __FILE__, __LINE__, b, x, y,p,q, z);
;goto illegal_op;
#else
;DPRINTF("[%s:%d] PARTIAL - PREFIX_ED case, byte 0x%02x (x %d, y %d [p=%d/q=%d], z %d) retrieved\n", __FILE__, __LINE__, b, x, y,p,q, z);
#endif
//        switch (x) {
//        case 0:
//            zprintf("nop\n");
