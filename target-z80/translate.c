...
;fprintf(stderr, "%s(): set pc_ptr <- pc_start 0x%04x\n", __func__, pc_ptr);
    ...
    lj = -1;
#if 1	/* WmT - TRACE */
;fprintf(stderr, "%s(): PARTIAL - not considering model (missing in env?)\n", __func__);
#endif
//    dc->model = env->model;
...
#if 1	/* WmT - TRACE */
;printf("[%s:%d] decided on max_insns %d\n", __FILE__, __LINE__, max_insns);
#endif
...
;printf("[%s:%d] About to disas_insn() at pc_ptr=%04x...\n", __FILE__, __LINE__, pc_ptr);
#endif
        pc_ptr = disas_insn(dc, pc_ptr);
        num_insns++;
#if defined(TARGET_Z80)
;printf("[%s:%d] next PC 0x%04x, after %d insns; +ve is_jmp (%d) will stop translation\n", __FILE__, __LINE__, pc_ptr, num_insns, dc->is_jmp);
#endif
        /* stop translation if indicated */
        if (dc->is_jmp) {
#if 1	/* WmT - TRACE */
;fprintf(stderr, "%s(): is_jmp %d -> stop translation\n", __func__, dc->is_jmp);
#endif
            break;
        }
...
#ifdef DEBUG_DISAS
;fprintf(stderr, "** %s(): PARTIAL - handle DEBUG_DISAS via log_target_disas() **\n", __func__);
...
{
#if 0	/* WmT - PARTIAL */
;fprintf(stderr, "BAIL %s() - INCOMPLETE\n", __func__);
;exit(1);
#else
