...
#if 1	/* WmT - HACK */
;fprintf(stderr, "[%s:%d] HACK - PREFIX_CB case, byte 0x%02x (x %d, y %d [p=%d/q=%d], z %d) -> unhandled\n", __FILE__, __LINE__, b, x, y,p,q, z);
;goto illegal_op;
#else
;fprintf(stderr, "[%s:%d] PARTIAL - PREFIX_CB case, byte 0x%02x (x %d, y %d [p=%d/q=%d], z %d) retrieved\n", __FILE__, __LINE__, b, x, y,p,q, z);
#endif
//        if (m != MODE_NORMAL) {
...
#if 1	/* WmT - HACK */
;fprintf(stderr, "[%s:%d] HACK - PREFIX_ED case, byte 0x%02x (x %d, y %d [p=%d/q=%d], z %d) -> unhandled\n", __FILE__, __LINE__, b, x, y,p,q, z);
;goto illegal_op;
#else
;fprintf(stderr, "[%s:%d] PARTIAL - PREFIX_ED case, byte 0x%02x (x %d, y %d [p=%d/q=%d], z %d) retrieved\n", __FILE__, __LINE__, b, x, y,p,q, z);
#endif
//        switch (x) {
//        case 0:
//            zprintf("nop\n");
